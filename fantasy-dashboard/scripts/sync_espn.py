    ...
    for league_id in LEAGUE_IDS:
        ...
        result = conn.execute(
            ...
            """),
            {
                ...
                "scoring_enhancement_type": settings.get("scoringSettings", {}).get(
                    "scoringType"
                ),
                "team_count": settings.get("size"),
                "teams_joined": status.get("numberOfTeams"),
                "draft_type": settings.get("draftSettings", {}).get("type"),
                "time_per_selection": settings.get("draftSettings", {}).get(
                    "timePerSelection"
                ),
                "faab_budget": settings.get("acquisitionSettings", {}).get(
                    "acquisitionBudget"
                ),
                "waiver_type": settings.get("acquisitionSettings", {}).get(
                    "waiverType"
                ),
        ...
        conn.execute(
            text("delete from league_slots where league_id = :league_id"),
...
def sync_players(conn):
    ...
    for item in items:
        ...
        eligible_names = "|".join(
            SLOT_MAP.get(int(slot), f"slot_{slot}")
